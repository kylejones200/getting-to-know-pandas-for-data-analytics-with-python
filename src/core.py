    ...
    for op in operations:
        if op == 'groupby':
            if 'category' in result.columns:
                result = result.groupby('category').agg({col: 'mean' for col in result.select_dtypes(include=[np.number]).columns})
        elif op == 'sort':
...
def analyze_dataframe(df: pd.DataFrame) -> Dict:
    ...
    return {'info': df.info(), 'head': df.head(), 'tail': df.tail(), 'numeric_columns': list(df.select_dtypes(include=[np.number]).columns), 'categorical_columns': list(df.select_dtypes(include=['object']).columns), 'missing_values': df.isnull().sum().to_dict()}

def plot_dataframe_comparison(df1: pd.DataFrame, df2: pd.DataFrame, column: str, title: str, output_path: Path, plot: bool=False):
    ...
    if column in df1.columns and column in df2.columns:
        if df1[column].dtype in ['int64', 'float64']:
            ax.hist(df1[column].dropna(), bins=30, alpha=0.6, label='Original', color='#4A90A4', edgecolor='none')
            ax.hist(df2[column].dropna(), bins=30, alpha=0.6, label='Processed', color='#D4A574', edgecolor='none')
        else:
