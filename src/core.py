        ...
        if op == "groupby":
            ...
            if len(result.select_dtypes(include=[np.number]).columns) > 0:
                result = result.sort_values(
                    by=result.select_dtypes(include=[np.number]).columns[0],
                    ascending=False,
                )
        elif op == "filter":
            if len(result.select_dtypes(include=[np.number]).columns) > 0:
                result = result[
                    result[result.select_dtypes(include=[np.number]).columns[0]]
                    > result[
                        result.select_dtypes(include=[np.number]).columns[0]
                    ].median()
                ]
